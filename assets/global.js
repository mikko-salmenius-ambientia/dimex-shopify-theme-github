...
// Quick Order List functionality starts here
...
class QuickOrderList extends HTMLElement {
  ...
  updateMultipleQty(items) {
    this.querySelector('.variant-remove-total .loading-spinner').classList.remove('hidden');
    ...
    fetch(`${routes.cart_update_url}`, { ...fetchConfig(), ...{ body } })
      ...
      .finally(() => {
        this.querySelector('.variant-remove-total .loading-spinner').classList.add('hidden');
      });
  }
    ...
    fetch(`${routeUrl}`, { ...fetchConfig(fetchConfigType), ...{ body } })
      ...
      .then((state) => {
        ...
        if (parsedState.description || parsedState.errors) {
          const variantItem = document.querySelector(`[id^="Variant-${id}"] .variant-item__totals.small-hide .loading-spinner`);
          variantItem.classList.add('loading-spinner--error');
  ...
  toggleLoading(id, enable) {
    ...
    const quickOrderListItems = this.querySelectorAll(`#Variant-${id} .loading-spinner`);
